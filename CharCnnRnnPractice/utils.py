...
# this function is init the weights of models
def init_weights(m):
...
def extract_char_cnn_rnn_weights(model_path, dataset, model_type):
    ...
    '''
    assert dataset in ['birds', 'flowers'], 'Dataset must be (birds|flowers)'
    assert dataset in ['cvpr', 'icml'], 'Dataset must be (cvpr|icml)'
