...
class DecoderLayer(nn.Module):
    ...
    def __init__(self):
        ...
        Inputs: 
            x (tensor): [B, seq_len, d_model]. 
            e_output (tensor): [B, seq_len, d_model]. 
            e_mask (tensor): [B, 1, seq_len]. 
            d_mask (tensor): [B, seq_len, seq_len]. 
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self):
        ...
        Inputs:
            q (tensor): [B, L, d_model]. 
            k (tensor): [B, L, d_model]. 
            v (tensor): [B, L, d_model]. 
            mask (tensor): [B, 1, L] or [B, L, L]. 
        '''
        super().__init__()
        self.inf = 1e9
    ...
    def self_attention(self, q, k, v, mask=None):
        '''
        Block for self attention.

        Inputs:
            q (tensor): [B, num_heads, L, d_k]. 
            k (tensor): [B, num_heads, L, d_k]. 
            v (tensor): [B, num_heads, L, d_k]. 
            mask (tensor): [B, 1, L] or [B, L, L]. 
        
        Returns:
            attn_values: [B, num_heads, L, d_k]. 
        '''
