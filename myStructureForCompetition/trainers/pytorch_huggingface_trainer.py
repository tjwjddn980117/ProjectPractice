...
def train(model, epoch, total_epoch, train_dataloader, criterion, optimizer):
    ...
    for i, (data, target) in progress_bar:
        ...
        optimizer.zero_grad()
        output, _ = model(data)
        #output = model(data).pooler_output
...
def trainer(load_file=False):
    ...
    for fold_idx, (train_index, val_index) in enumerate(skf.split(train_df, train_df['class'])):
        ...
        model = CustomModel(timm.create_model('eva_large_patch14_196.in22k_ft_in22k_in1k', pretrained=True).to(CFG.DEVICE)).to(CFG.DEVICE)
        ...
        for epoch in range(CFG.EPOCHS):
            ...
            if val_loss < best_val_loss:
                ...
                best_checkpoint_path = os.path.join(checkpoint_dir, 'eva_large_patch14_196.pt')
