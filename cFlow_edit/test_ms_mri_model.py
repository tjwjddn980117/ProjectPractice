...
from data.dataset_config import exp_config_ms_mri
...
def main():
    ...
    lidc = MS_MRI_Dataloader(exp_config_ms_mri)
    ...
    if args.flow:
        ...
        fName = fName+'_flow'
        net = cFlowNet(input_channels=4, num_classes=1, 
        ...
        fName = fName+'_Unet'
        net = Unet(input_channels=4, num_classes=1, 
    ...
    load_model_path = 'C:/Users/Seo/Downloads/CFlow_/20241014_16_07_4R_glow_MS_MRI.pt'
    ...
    with torch.no_grad():
        ...
        for step, (patch, masks, pred_prob, path) in enumerate(tqdm(test_loader)):
            ...
            if not args.unet:
                ...
                save_results_with_batch(samples_tensor, path, 'C:/Users/Seo/Downloads/cFlow-master/Results_MS_MRI')
                #differences = []
                #for i in range(1, 16):
