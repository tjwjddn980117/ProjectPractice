...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        dGED = 0
        t = time.time()
        for step, (patch, masks, pred_prob, path) in enumerate(train_loader): 
            ...
            if (step+1) % 5 == 0:
                with torch.no_grad():
                    for idx, (patch, masks, pred_prob, path) in enumerate(valid_loader): 
                        patch = patch.to(device)
                        masks = masks.to(device)
                            # Choose a random mask
                        mask = masks[:,[rater]]
        ...
        if (epoch+1) % 1 == 0 and epValidLoss > 0 and epValidLoss < minLoss:
            ...
            if not os.path.exists('../models'):
                os.makedirs('../models')
            torch.save(net.state_dict(),'../models/'+fName+'_MS_MRI.pt')
        else:
            convIter += 1
